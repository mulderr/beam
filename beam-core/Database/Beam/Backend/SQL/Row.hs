...
module Database.Beam.Backend.SQL.Row
  ...
  , parseOneField, peekField, failParse

  , ColumnParseError(..), BeamRowReadError(..)

  , FromBackendRow(..), valuesNeeded'
  ) where
...
data FromBackendRowF be f where
  ...
  Alt :: FromBackendRowA be a -> FromBackendRowA be a -> (a -> f) -> FromBackendRowF be f
  FailParseWith :: BeamRowReadError -> FromBackendRowF be f
deriving instance Functor (FromBackendRowF be)

newtype FromBackendRowA be a = FromBackendRowA (Ap (FromBackendRowF be) a)
  deriving (Functor, Applicative)

instance Alternative (FromBackendRowA be) where
  empty = failParse "empty"
  a <|> b =
    FromBackendRowA (liftAp (Alt a b id))

parseOneField :: (BackendFromField be a, Typeable a) => FromBackendRowA be a
parseOneField =
  FromBackendRowA (liftAp (ParseOneField id))

peekField :: (Typeable a, BackendFromField be a) => FromBackendRowA be (Maybe a)
peekField = fmap Just (FromBackendRowA (liftAp (ParseOneField id))) <|> pure Nothing

failParse :: String -> FromBackendRowA be a
failParse =
  FromBackendRowA . liftAp . FailParseWith .
...
class BeamBackend be => FromBackendRow be a where
  ...
  fromBackendRow :: FromBackendRowA be a
  default fromBackendRow :: (Typeable a, BackendFromField be a) => FromBackendRowA be a
...
valuesNeeded' (FromBackendRowA x) = getSum $ runAp_ (\_ -> Sum 1) x

class GFromBackendRow be (exposed :: * -> *) rep where
  gFromBackendRow :: Proxy exposed -> FromBackendRowA be (rep ())
...
instance ( BeamBackend be, KnownNat n, FromBackendRow be a ) => FromBackendRow be (Vector n a) where
  fromBackendRow = maybe (error "impossible") id . Vector.fromListN <$> replicateM (fromIntegral $ natVal (Proxy @n)) fromBackendRow
  ...
  fromBackendRow =
    ...
    (Nothing <$
      replicateM_ (valuesNeeded (Proxy @be) (Proxy @(Maybe x)))
                  (fromBackendRow :: FromBackendRowA be SqlNull))
